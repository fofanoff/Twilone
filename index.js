...
app.use(session({
    secret: process.env.TWILONE_SESSION_SECRET,
    resave: false,
...
app.post('/register', (request, response) => {
    ...
    User.create({
        'login' : login,
        'password' : bcrypt.hashSync(password, parseInt(process.env.TWILONE_PASSWORD_SALT_ROUND))
    }).then(user => {
...
sequelize.sync().then(() => {
    User.upsert({
        ...
        'password' : bcrypt.hashSync(process.env.TWILONE_ADMIN_PASSWORD, parseInt(process.env.TWILONE_PASSWORD_SALT_ROUND)),
        'admin' : true
    }).then(() => {
